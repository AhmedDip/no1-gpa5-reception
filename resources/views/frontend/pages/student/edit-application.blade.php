            ...
            <div class="card shadow">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('student.update.application') }}" method="POST" enctype="multipart/form-data" id="editForm">
                        @csrf
                        <div class="row">
                            <div class="col-md-6 mb-3"><label class="form-label required">ইংরেজিতে নাম</label><input type="text" class="form-control" name="name_en" value="{{ $studentDetail->name_en }}" required></div>
                            <div class="col-md-6 mb-3"><label class="form-label required">বাংলায় নাম</label><input type="text" class="form-control" name="name_bn" value="{{ $studentDetail->name_bn }}" required></div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3"><label class="form-label required">এসএসসি বোর্ড</label>
                                <select class="form-control" name="ssc_board_id" required>
                                    ...
                                    @foreach($sscBoards as $board)
                                        <option value="{{ $board->id }}" {{ $studentDetail->ssc_board_id == $board->id ? 'selected' : '' }}>{{ $board->name_bn }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3"><label class="form-label required">গ্রুপ</label>
                                <select class="form-control" name="student_group_id" required>
                                    ...
                                    @foreach($studentGroups as $group)
                                        <option value="{{ $group->id }}" {{ $studentDetail->student_group_id == $group->id ? 'selected' : '' }}>{{ $group->name_bn }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3"><label class="form-label required">রোল নম্বর</label><input type="text" class="form-control" name="roll_number" value="{{ $studentDetail->roll_number }}" required></div>
                            <div class="col-md-6 mb-3"><label class="form-label required">রেজিস্ট্রেশন নম্বর</label><input type="text" class="form-control" name="registration_number" value="{{ $studentDetail->registration_number }}" required></div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3"><label class="form-label required">জিপিএ/ফলাফল</label><input type="text" class="form-control" name="gpa_result" value="{{ $studentDetail->gpa_result }}" required></div>
                        </div>
                        <div class="row">
                            <div class="col-md-4 mb-3"><label class="form-label required">বিভাগ</label>
                                <select class="form-control" name="division_id" id="division_id" required>
                                    ...
                                    @foreach($divisions as $division)
                                        <option value="{{ $division->id }}" {{ $studentDetail->division_id == $division->id ? 'selected' : '' }}>{{ $division->name_bn }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4 mb-3"><label class="form-label required">জেলা</label>
                                <select class="form-control" name="district_id" id="district_id" required>
                                    ...
                                    @foreach($districts as $district)
                                        <option value="{{ $district->id }}" {{ $studentDetail->district_id == $district->id ? 'selected' : '' }}>{{ $district->name_bn }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4 mb-3"><label class="form-label required">উপজেলা</label>
                                <select class="form-control" name="upazila_id" id="upazila_id" required>
                                    ...
                                    @foreach($upazilas as $upazila)
                                        <option value="{{ $upazila->id }}" {{ $studentDetail->upazila_id == $upazila->id ? 'selected' : '' }}>{{ $upazila->name_bn }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        @if($studentDetail->student_photo)<div class="mt-2"><img src="{{ asset('storage/' . $studentDetail->student_photo) }}" style="max-width: 150px; border-radius: 10px;"><small class="text-muted d-block">বর্তমান ছবি</small></div>@endif
