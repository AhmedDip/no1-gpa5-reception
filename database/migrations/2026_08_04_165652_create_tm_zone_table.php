<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('tm_zone', function (Blueprint $table) {
            $table->id();
            $table->string('zone_name', 255);
            $table->string('zone_code', 255)->unique();
            $table->integer('dirg_id');
            $table->string('dirg_code', 255)->default('-');
            $table->integer('aemp_id');
            $table->integer('cont_id');
            $table->integer('lfcl_id');
            $table->integer('aemp_iusr');
            $table->integer('aemp_eusr');
            $table->timestamp('created_at')->useCurrent();
            $table->timestamp('updated_at')->useCurrent()->useCurrentOnUpdate();
            $table->tinyInteger('var')->default(1);
            $table->string('attr1', 255)->default('-');
            $table->string('attr2', 5)->default('-');
            $table->integer('attr3')->default(0);
            $table->integer('attr4')->default(0);


            $table->index('aemp_eusr');
            $table->index('aemp_iusr');
            $table->index('lfcl_id');
            $table->index('cont_id');
            $table->index('dirg_id');
            $table->index('zone_code');
            $table->index('zone_name');
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('tm_zone');
    }
};
